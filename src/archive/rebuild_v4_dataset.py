...
def main():
    ...
    print("\nFinal Distribution (v4):")
    print(train_df['class'].value_counts())
    
    # 8. Save
    train_df.to_csv('data/train_final_v4.csv', index=False, encoding='utf-8-sig')
    val_df.to_csv('data/val_final_v4.csv', index=False, encoding='utf-8-sig')
    
    print("\nDataset Reconstruction Complete (v4)!")
    print(f"Train: data/train_final_v4.csv ({len(train_df)} rows)")
    print(f"Val: data/val_final_v4.csv ({len(val_df)} rows)")
