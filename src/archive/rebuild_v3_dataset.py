...
def forensic_clean(txt):
    ...
    # This is critical for V3 which has A:, B: prefix
...
def main():
    ...
    # Target: ~3000-4000 per class balanced
    ...
    print("\nFinal Distribution (v3):")
    print(train_df['class'].value_counts())
    
    # 8. Save
    train_df.to_csv('data/train_final_v3.csv', index=False, encoding='utf-8-sig')
    val_df.to_csv('data/val_final_v3.csv', index=False, encoding='utf-8-sig')
    
    print("\nDataset Reconstruction Complete (v3)!")
    print(f"Train: data/train_final_v3.csv ({len(train_df)} rows)")
    print(f"Val: data/val_final_v3.csv ({len(val_df)} rows)")
